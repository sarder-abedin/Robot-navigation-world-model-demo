...
| `CMD_AIMOVE` | PC → Pi | `CMD_AIMOVE#<FORWARD\|SLOW\|STOP\|REROUTE\|BACKUP\|TURN>[#<LEFT\|RIGHT>]` | AI-computed action; Pi maps to motor PWM. REROUTE = back-up+spin (depth's open side); BACKUP = short reverse; **TURN = in-place spin toward the goal (Goal-Following, no backup)** |
...
| `CMD_AISTATUS` | PC → UI | `CMD_AISTATUS#<action>#<risk_pct>#<wm_label>#<pattern>#<sonic_cm>#<ssv2_sentence>#<clear_dist_m>#<clear_dir>#<goal_status>#<depth_left_m>#<depth_right_m>#<goal_bearing_deg>#<goal_dist_m>#<pose_x_m>#<pose_y_m>#<pose_heading_deg>` | Live AI state for the UI panel + both navigation maps. `goal_status` ∈ none/tracking/lost/reached; on **reached** the PC stops the robot until a new UI command. The trailing depth_left/right + goal bearing/distance drive the egocentric map; the final **pose_x/pose_y/pose_heading** (dead-reckoned, no odometry → drifts) anchor the world map (-1 = unknown). Trailing fields optional for old clients |
| `CMD_MAPOBJ` | PC → UI | `CMD_MAPOBJ#<label>,<bearing_deg>,<dist_m>;…` | Per-frame YOLO objects for the world map: `bearing_deg` from the box centre (+right/−left), `dist_m` from the depth model (-1 = unknown). Companion to `CMD_AISTATUS`; the UI projects each into the world frame with the pose and flags moving ones. Empty → bare `CMD_MAPOBJ#` |
