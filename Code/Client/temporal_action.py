"""
temporal_action.py – SSv2-style motion pattern recognition (client side).

Runs on the operator PC alongside V-JEPA 2 and the decision fuser.
Identical logic to Code/Server/temporal_action.py — split here so the
client can import it without touching the Pi server's Python path.

See Code/Server/temporal_action.py for full documentation.
"""
...
class TemporalActionRecognizer:
    ...
    Rule-based (no GPU) — runs every frame at negligible cost.
    """
