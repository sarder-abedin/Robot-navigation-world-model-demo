...
| `Code/Server/world_model_process.py` | Runs V-JEPA 2 + SSv2 in a **separate OS process** (`spawn`) so their multi-second, GIL-holding forwards don't freeze the main server's camera-receive I/O (which stalled the robot stream). Lock-step clip-in/result-out over `multiprocessing.Queue`; `world_model.run_in_subprocess: false` falls back to an in-process thread. Because the worker is forward-bound (never queues), it runs **V-JEPA 2 on every clip** for the freshest risk (the `run_every_n_frames` skip-gate is only for the in-process path) and SSv2 on a small job cadence (`ssv2.subprocess_run_every`, default 2) |
...
| `Code/Server/build_anchor_set.py` | **Auto-sorts a balanced blocked/clear frame set** (skip manual sorting) from either a logged run (`--run`, independent YOLO+sonar+action labels, prefers `raw_frames/`) or a video (`--video`, YOLO-labelled) → `<out>/blocked` + `<out>/clear`; `--build`/`--apply` can also build+wire the anchors. Pure label/balance core is unit-tested |
...
| `Code/Server/run_report.py` | Offline matplotlib plots + summary of `navigation_log.csv` (risk/distance/action/latency/network); `analyze()` → insights + plain-language auto-notes (risk-driver split, uncalibrated anchors, ultrasonic-driven, YOLO-quiet, no-echo, dropped-frames, latency); **multi-run compare** (`build_compare_figures()`/`compare_table()` overlay N runs); `save_pngs()` → `<run>/viz/`; `save_report()` → self-contained shareable **HTML** (embedded charts + findings, single or comparison). Backend-agnostic + unit-tested |
| `Code/Server/run_visualizer.py` | **Desk-only PyQt5 run visualizer** (`python run_visualizer.py`): select **one run** → embedded plots + auto-analysis notes + synced annotated-frame scrubber, or **several runs** → overlaid comparison charts + per-run findings → Save PNGs / **Save report (HTML)** |
| `Code/Robot/calibrate_governor.py` | On-robot: measures governor m/s constants (sonar+motors), safely patches `config.yaml` |
| `Code/Server/visualization.py` | HUD overlay: keeps spatial cues on the video (boxes, risk bar, action, depth L/C/R bars, goal marker+arrow+readout); text overlays (V-JEPA2/motion, sonic, fps, ssv2, depth-distance) default OFF and are shown in the UI panel below the video instead. Renders the **"what V-JEPA 2 sees" dense-feature map** two ways (`world_model.feature_viz_mode`): `_overlay_feature_on()` blends it **semi-transparently onto the camera** (default; drawn under the HUD so the HUD stays crisp, `feature_viz_alpha`) or `_append_feature_panel()` hstacks it **beside** the camera; both cubic-upscaled + blurred into smooth regions |
| `Code/Server/feature_viz.py` | Pure-numpy PCA of V-JEPA 2's dense patch features → an RGB image (paper Fig 1 technique): `patch_features_to_rgb()` (SVD top-3 → RGB, sign-aligned across frames to stop flicker, **desaturated toward luminance** via `saturation` so it reads as coherent regions not a rainbow) + `infer_patch_grid()`. No torch → **unit-tested** (`tests_rpi/test_feature_viz_rpi.py`); `world_model._compute_feature_rgb()` feeds it, the HUD shows it overlaid/side (see visualization.py), toggled live by `CMD_FEATUREVIZ` (`world_model.feature_viz` config, on by default) |
